...
import { FaCode, FaRobot, FaBolt, FaKeyboard, FaShare, FaEye, FaEyeSlash, FaHammer, FaRocket, FaArrowRotateLeft, FaArrowRotateRight, FaGear, FaTableCellsLarge, FaDiagramProject } from 'react-icons/fa6';
...
}> = ({ projectName, undo, redo, canUndo, canRedo, appMode, onModeChange, onShareClick, onPublishClick, onSettingsClick }) => {
  return (
    <header className="h-16 bg-[var(--color-surface)] border-b border-[var(--color-border)] flex items-center p-4 text-white justify-between flex-shrink-0 z-40">
          ...
          <div className="bg-[var(--color-background)] rounded-lg p-1 flex">
            {(['design', 'logic', 'code', 'devtools'] as AppMode[]).map(mode => (
              <Tooltip key={mode} content={`${mode.charAt(0).toUpperCase() + mode.slice(1)} View`}>
                ...
                >
                  {mode === 'design' && <FaTableCellsLarge />}
                  {mode === 'logic' && <FaDiagramProject />}
                  {mode === 'code' && <FaCode />}
                  {mode === 'devtools' && <FaHammer />}
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2">
            <button onClick={onShareClick} className="px-3 py-2 bg-[var(--color-surface-light)] hover:bg-[var(--color-border)] rounded-md text-sm font-semibold flex items-center gap-2">
              <FaShare size={16} /> Share
            </button>
            <button onClick={onPublishClick} className="px-3 py-2 bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)] rounded-md text-sm font-semibold flex items-center gap-2">
              <FaRocket size={16} /> Publish
            </button>
        </div>
      </div>
    </header>
